...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="flex items-start justify-between gap-3"><div><h1 class="text-xl font-semibold">Characters</h1><p class="text-sm text-muted">Personnages du projet (CRUD + corbeille).</p></div><label class="text-sm text-muted flex items-center gap-2 select-none"><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(trashed.value) ? ssrLooseContain(trashed.value, null) : trashed.value) ? " checked" : ""}> Trashed </label></div><div class="card"><div class="card-body grid gap-3 md:grid-cols-4"><div class="md:col-span-1"><label class="text-xs text-muted">Search</label><input${ssrRenderAttr("value", q.value)} class="input w-full" placeholder="name / slug…"></div><div class="md:col-span-1"><label class="text-xs text-muted">New name</label><input${ssrRenderAttr("value", form.name)} class="input w-full" placeholder="e.g. Kinuani"></div><div class="md:col-span-1"><label class="text-xs text-muted">Slug (optional)</label><input${ssrRenderAttr("value", form.slug)} class="input w-full" placeholder="kinuani"></div><div class="md:col-span-1"><label class="text-xs text-muted">Description (optional)</label><input${ssrRenderAttr("value", form.description)} class="input w-full" placeholder="2-3 mots, ou laisse vide…"></div><div class="md:col-span-4"><button class="btn btn-primary w-full focus-ring"${ssrIncludeBooleanAttr(creating.value) ? " disabled" : ""}>`);
      ...
      if (unref(pending)) {
        ...
        _push(`<span>${ssrInterpolate(unref(data)?.characters?.length || 0)} character(s)</span>`);
      }
      _push(`</div></div><div class="divide-y divide-border"><!--[-->`);
      ssrRenderList(unref(data)?.characters || [], (c) => {
        _push(ssrRenderComponent(_component_NuxtLink, {
          key: c.slug,
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="flex items-start justify-between gap-3"${_scopeId}><div${_scopeId}><div class="font-extrabold"${_scopeId}>${ssrInterpolate(c.name)}</div><div class="text-xs text-muted mt-1"${_scopeId}>${ssrInterpolate(c.slug)}</div>`);
              if (c.description) {
                _push2(`<div class="text-sm text-muted mt-1"${_scopeId}>${ssrInterpolate(c.description)}</div>`);
              } else {
                _push2(`<!---->`);
              }
              ...
              return [
                createVNode("div", { class: "flex items-start justify-between gap-3" }, [
                  createVNode("div", null, [
                    createVNode("div", { class: "font-extrabold" }, toDisplayString(c.name), 1),
                    createVNode("div", { class: "text-xs text-muted mt-1" }, toDisplayString(c.slug), 1),
                    c.description ? (openBlock(), createBlock("div", {
                      key: 0,
                      class: "text-sm text-muted mt-1"
                    }, toDisplayString(c.description), 1)) : createCommentVNode("", true)
                  ]),
                  createVNode("div", { class: "flex items-center gap-2" }, [
                    c.deleted_at ? (openBlock(), createBlock("span", {
                      key: 0,
                      class: "badge"
                    }, "trashed")) : createCommentVNode("", true),
                    !c.deleted_at ? (openBlock(), createBlock("button", {
                      key: 1,
                      class: "btn btn-ghost focus-ring",
                      onClick: withModifiers(($event) => softDelete(c), ["prevent", "stop"])
                    }, [
                      ...
                      key: 2,
                      class: "btn btn-ghost focus-ring",
                      onClick: withModifiers(($event) => restore(c), ["prevent", "stop"])
                    }, [
                      createVNode(_component_Icon, {
                        name: "mdi:backup-restore",
                        "aria-hidden": "true"
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/projects/[slug]/characters/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=index-DayVVlYE.mjs.map
