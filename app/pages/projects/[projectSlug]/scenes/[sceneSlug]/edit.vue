<template>
  <div class="max-w-xl mx-auto p-8">
    <h1 class="text-2xl font-bold mb-6">Éditer la scène</h1>
    <form v-if="loaded" @submit.prevent="submitForm" class="space-y-4">
      ...
      <div v-if="success" class="text-green-600 mt-2">Scène modifiée avec succès !</div>
    </form>
...
onMounted(async () => {
  try {
    ...
    if (!res.ok) throw new Error('Scène introuvable')
...
async function submitForm() {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Erreur lors de la modification de la scène')
    success.value = true
    setTimeout(() => {
      router.push(`/projects/${projectSlug}/scenes`)
    }, 1200)
  } catch (e) {
    error.value = e.message
  } finally {
...
.btn {
  ...
  padding-right: 1rem;
  border-radius: 0.25rem;     /* Tailwind rounded */
  transition: background-color 0.2s; /* Tailwind transition */
}
.btn:hover {
