<template>
  <div class="max-w-xl mx-auto p-8">
    <h1 class="text-2xl font-bold mb-6">Éditer l'événement</h1>
    <form v-if="loaded" @submit.prevent="submitForm" class="space-y-4">
      ...
      <div v-if="success" class="text-green-600 mt-2">Événement modifié avec succès !</div>
    </form>
...
async function submitForm() {
  ...
  try {
    ...
    success.value = true
    setTimeout(() => {
      router.push(`/projects/${projectSlug}/timeline`)
    }, 1200)
  } catch (e) {
    error.value = e.message
  } finally {
