...
const timelineEvents = computed(() => timelineData.value?.events || [])
...
watchEffect(() => {
  ...
  form.timeline_event_id = s.timeline_event_id ? String(s.timeline_event_id) : ''
...
function chapterLabel(id, chapterNo, title, partNo) {
  const partPrefix = partNo ? `P${partNo} ` : ''
  const base = chapterNo !== null && chapterNo !== undefined ? `Chapter ${chapterNo}` : `Chapter #${id}`
  const prefix = `${partPrefix}${base}`
...
function eventLabel(e) {
  return e?.title || e?.name || e?.label || e?.event || e?.slug || String(e?.id || '')
}
  ...
  try {
    await apiFetch(`/api/projects/${projectSlug.value}/scenes/${sceneSlug.value}`, {
      ...
      body: {
        ...
        timeline_event_id: Number.isFinite(Number(form.timeline_event_id)) ? Number(form.timeline_event_id) : null,
    ...
    <div class="card">
      <div class="card-body space-y-4">
        ...
        <div class="grid gap-3 md:grid-cols-4">
          <div>
            ...
            <select v-model="form.chapter_id" class="input w-full" :disabled="isTrashed">
              ...
              <option v-for="c in chapters" :key="c.id" :value="String(c.id)">
                {{ chapterLabel(c.id, c.chapter_no, c.title, c.part_no) }}
              </option>
            </select>
          </div>
          ...
          <div>
            <label class="text-xs text-muted">Timeline event</label>
            <select v-model="form.timeline_event_id" class="input w-full" :disabled="isTrashed">
              <option value="">—</option>
              <option v-for="e in timelineEvents" :key="e.id || e.slug" :value="String(e.id || e.slug)">
                {{ eventLabel(e) }}
              </option>
            </select>
          </div>
