...
export default defineEventHandler(async (event) => {
  ...
  const { q, chapter_id } = getQuery(event)
  ...
  const rows = await dbQuery(
    ...
    SELECT
      ...
      s.objective, s.time_of_day, s.summary, s.conflict, s.turning_point,
    ...
    WHERE s.project_id = ?
      ...
      AND (? IS NULL OR s.title LIKE CONCAT('%', ?, '%') OR s.slug LIKE CONCAT('%', ?, '%') OR s.summary LIKE CONCAT('%', ?, '%'))
